...
internal bool serial_init(U16 port) {
    asm_port_outb(port + SERIAL_PO_INTERRUPT_ENABLE_REGISTER, 0x00); // disable interrupts
    asm_port_outb(port + SERIAL_PO_LINE_CONTROL_REGISTER,     0x80); // enable DLAB to set baud divisor
    asm_port_outb(port + SERIAL_PO_LEAST_DIVISOR,             0x03); // set divisor to 3, 38400 baud
    asm_port_outb(port + SERIAL_PO_MOST_DIVISOR,              0x00); // high byte
    asm_port_outb(port + SERIAL_PO_LINE_CONTROL_REGISTER,     0x03); // 8 bits, no parity, one stop bit
    asm_port_outb(port + SERIAL_PO_FIFO_CONTROL_REGISTER,     0xC7); // enable & clear fifo, 14-byte threshold
    asm_port_outb(port + SERIAL_PO_MODEM_CONTROL_REGISTER,    0x0B); // IRQs enabled, RTS/DSR set

    // test serial
    asm_port_outb(port + SERIAL_PO_MODEM_CONTROL_REGISTER, 0x1E); // enable loopback
    asm_port_outb(port + SERIAL_PO_TRANSMIT_BUFFER,        0xAE); // enable loopback

    if (asm_port_inb(port + SERIAL_PO_RECEIVE_BUFFER) != 0xAE) {
        return false;
    }

    asm_port_outb(port + SERIAL_PO_MODEM_CONTROL_REGISTER, 0x0F); // IRQs enabled, out 1 & 2 enabled
    return true;
}

internal bool serial_read_available(U16 port) {
    return !!(asm_port_inb(port + SERIAL_PO_LINE_STATUS_REGISTER) & 0x01);
}

internal bool serial_write_available(U16 port) {
    return !!(asm_port_inb(port + SERIAL_PO_LINE_STATUS_REGISTER) & 0x20);
}

internal U8 serial_read_blocking(U16 port) {
    while (!serial_read_available(port));

    return asm_port_inb(port);
}

internal void serial_write_blocking(U16 port, U8 data) {
    while (!serial_write_available(port));

    asm_port_outb(port, data);
}
...
internal STREAM_FUNCTION(serial_stream_func) {
    ...
    case STREAM_OP_READ:
        ...
        while (serial_read_available(port) && n < buffer.len) {
            bytes_set(buffer, n, asm_port_inb(port));
            n             += 1;
        }
        return n;
    }
